...
1. **Tenancy in DB, not client.** Every query filtered by `company_id`. RLS: gestor bypass, empresa `company_id = own`, funcionario `user_id = own`. Never trust client-side filtering. JWT claims `role`, `company_id` set by the `set-claims` DB trigger on `profiles` (employee invites go through a separate admin-only function).
